...
function getDerivedKey(): Buffer {
  ...
  if (!secret || secret.trim().length === 0) {
    if (process.env.NODE_ENV === "production") {
      throw new Error(
        "[FATAL SECURITY CONFIGURATION] ENCRYPTION_SECRET (or JWT_SECRET) is missing in production. Encryption subsystem failed closed."
      );
    }
    return crypto.createHash("sha256").update("dubaifinance-dev-only-secure-salt-2026").digest();
  }

  if (process.env.NODE_ENV === "production" && secret.length < 32) {
    throw new Error(
      "[FATAL SECURITY CONFIGURATION] ENCRYPTION_SECRET (or JWT_SECRET) must be at least 32 characters long for AES-256-GCM."
    );
  }
...
export function decryptSecret(cipherText: string | null | undefined): string | null {
  ...
  try {
    ...
    console.warn("Decryption failed for secret, returning null");
    return null;
  }
}
